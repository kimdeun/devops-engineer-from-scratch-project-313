...
import { useResourceContext, useStore, useTranslateLabel, useDataTableStoreContext, useDataTableColumnRankContext, useDataTableColumnFilterContext, } from 'ra-core';
import * as diacritic from 'diacritic';
import { FieldToggle } from "../../preferences/index.js";
export var ColumnsSelectorItem = function (_a) {
    ...
    var columnFilter = useDataTableColumnFilterContext();
    var translateLabel = useTranslateLabel();
    if (!source && (!label || typeof label !== 'string'))
        return null;
    var sourceOrLabel = source || label;
    var fieldLabel = translateLabel({
        label: typeof label === 'string' ? label : undefined,
        resource: resource,
        source: source,
    });
    var isColumnHidden = hiddenColumns.includes(sourceOrLabel);
    var isColumnFiltered = fieldLabelMatchesFilter(fieldLabel, columnFilter);
    ...
    return isColumnFiltered ? (React.createElement(FieldToggle, { key: columnRank, source: sourceOrLabel, label: fieldLabel, index: String(columnRank), selected: !isColumnHidden, onToggle: function () {
            return isColumnHidden
                ? setHiddenColumns(hiddenColumns.filter(function (column) { return column !== sourceOrLabel; }))
                : setHiddenColumns(__spreadArray(__spreadArray([], hiddenColumns, true), [sourceOrLabel], false));
        }, onMove: handleMove })) : null;
};
...
var fieldLabelMatchesFilter = function (fieldLabel, columnFilter) {
    return columnFilter
        ? diacritic
            .clean(fieldLabel)
            .toLowerCase()
